...
import { Employee } from '../../../api/EmployeesService';
    ...
    if (id) {
      Promise.all([
        ...
        .then(([employeeResponse, timeOffRequestsResponse]) => {
          const timeOffRequests = timeOffRequestsResponse.data;
    ...
    TimeOffRequestsService.deleteTimeOffRequest(requestId)
      .then(() => {
        setEmployee((prevEmployee) => {
          ...
          return {
            ...prevEmployee,
            timeOffRequests: prevEmployee.timeOffRequests!.filter(
    ...
    <Box sx={{ padding: 3 }}>
      ...
      {employee && (
        <Box>
          ...
          {employee.timeOffRequests!.length ? (
            employee.timeOffRequests!.map((timeOff) => (
              <Card key={timeOff._id} sx={{ marginTop: 2, boxShadow: 3 }}>
                <CardContent>
                  <Typography variant="body1">
                    Starts: {formatDate(timeOff.start_date)}
                  </Typography>
                  <Typography variant="body1">
                    Ends: {formatDate(timeOff.end_date)}
                  </Typography>
                  <Typography variant="body1">
                    Type: {timeOff.request_category_id.name}
                  </Typography>
                </CardContent>
                <CardActions>
                  <Button
                    ...
                    color="secondary"
                    sx={{ margin: "0 auto" }}
                    onClick={() => handleDeleteRequest(timeOff._id)}
                  >
                    Delete
                  </Button>
                </CardActions>
      ...
      <Link to="/employee">
        <Button variant="outlined" size="large" sx={{ margin: 2 }}>
          Go Back
        </Button>
      </Link>
      <Snackbar
